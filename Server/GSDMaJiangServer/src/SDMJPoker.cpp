#include "SDMJPoker.h"
void SDMJPoker::init(Json::Value& jsOpt) {
	IMJPoker::init(jsOpt);
	...
	for (uint8_t nCnt = 0; nCnt < 4; ++nCnt)
	{
		...
		{
			addCardToPoker(makeCardNumber(eCT_Hua, nValue));
		}
	}
...
uint8_t SDMJPoker::getCardByIdx(uint16_t nIdx, bool isReverse) {
	if (isReverse) {
...
void SDMJPoker::makeSpecialCard(std::vector<uint8_t>& vMakedCards) {
#ifndef _DEBUG
	return;
#endif // !_DEBUG
	//return;
