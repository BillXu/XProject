...
#include "SDMJRoom.h"
class SDMJRoomStateAskForRobotGang
...
public:
	...
	void update(float fDeta)override
	{
		if (getWaitTime() > 15.0f) {
			for (auto& ref : m_vWaitIdx) {
				auto pPlayer = (SDMJPlayer*)getRoom()->getPlayerByIdx(ref);
	...
	bool onMsg(Json::Value& prealMsg, uint16_t nMsgType, eMsgPort eSenderPort, uint32_t nSessionID)override
	{
		...
		auto pPlayer = (SDMJPlayer*)getRoom()->getPlayerBySessionID(nSessionID);
		uint8_t nRet = 0;
		do
		{
			if (pPlayer == nullptr)
			...
			auto pMJCard = (SDMJPlayerCard*)((IMJPlayer*)pPlayer)->getPlayerCard();
