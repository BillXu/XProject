#include "ARQMJRoom.h"
#include "ARQMJPlayer.h"
#include "CommonDefine.h"
#include "ARQMJRoomStateWaitReady.h"
#include "ARQMJRoomStateWaitPlayerChu.h"
#include "ARQMJRoomStateWaitPlayerAct.h"
#include "MJRoomStateStartGame.h"
#include "MJRoomStateGameEnd.h"
#include "ARQMJRoomStateDoPlayerAct.h"
#include "ARQMJRoomStateAfterChiOrPeng.h"
#include "ARQMJRoomStateAskForRobotGang.h"
#include "ARQMJRoomStateAskForPengOrHu.h"
...
bool ARQMJRoom::init(IGameRoomManager* pRoomMgr, uint32_t nSeialNum, uint32_t nRoomID, std::shared_ptr<IGameOpts> ptrGameOpts)
{
	...
	m_nR15 = 0;
	// add room state ;
	IGameRoomState* p[] = { new ARQMJRoomStateWaitReady(), new ARQMJRoomStateWaitPlayerChu(),new ARQMJRoomStateWaitPlayerAct(),
		new MJRoomStateStartGame(),new MJRoomStateGameEnd(),new ARQMJRoomStateDoPlayerAct(),new ARQMJRoomStateAskForRobotGang(),
		new ARQMJRoomStateAskForPengOrHu(), new ARQMJRoomStateAfterChiOrPeng()
	};
	for ( auto& pS : p )
	{
		addRoomState(pS);
...
IGamePlayer* ARQMJRoom::createGamePlayer()
{
	return new ARQMJPlayer();
}

void ARQMJRoom::packRoomInfo(Json::Value& jsRoomInfo)
{
...
void ARQMJRoom::visitPlayerInfo(IGamePlayer* pPlayer, Json::Value& jsPlayerInfo, uint32_t nVisitorSessionID)
{
	if (!pPlayer)
	{
		return;
	...
	jsPlayerInfo["extraTime"] = ((ARQMJPlayer*)pPlayer)->getExtraTime();
	...
	((ARQMJPlayer*)pPlayer)->getPlayerCard()->onVisitPlayerCardInfo(jsPlayerInfo, pPlayer->getSessionID() == nVisitorSessionID);
}

void ARQMJRoom::onWillStartGame() {
...
void ARQMJRoom::onStartGame()
{
	IMJRoom::onStartGame();
	sendStartGameMsg();
}

void ARQMJRoom::sendStartGameMsg() {
	Json::Value jsMsg;
	m_nDice = 1 + rand() % 6;
	...
	for (auto& pPlayer : m_vPlayers)
	{
		...
		Json::Value peer;
		auto pPlayerCard = ((ARQMJPlayer*)pPlayer)->getPlayerCard();
...
uint8_t ARQMJRoom::getRoomType()
{
	return eGame_ARQMJ;
}

IPoker* ARQMJRoom::getPoker()
{
	return &m_tPoker;
}

bool ARQMJRoom::isCanGoOnMoPai() {
	auto nLeftCnt = getPoker()->getLeftCardCount();
	return nLeftCnt > 0 && nLeftCnt + m_nGangCnt > 16;
}

bool ARQMJRoom::canGang() {
	auto nLeftCnt = getPoker()->getLeftCardCount();
	if (nLeftCnt > 3) {
...
bool ARQMJRoom::needChu() {
	return canGang();
}

void ARQMJRoom::onPrePlayerGang() {
	m_nGangCnt++;
...
void ARQMJRoom::onGameEnd() {
	...
	for (auto& ref : m_vPlayers) {
		if (ref) {
			...
			((ARQMJPlayer*)ref)->getPlayerCard()->getHoldCard(vHoldCard);
...
void ARQMJRoom::doProduceNewBanker() {
...
void ARQMJRoom::setNextBankerIdx(uint8_t nHuIdx) {
	if ((uint8_t)-1 == getBankerIdx()) {
		m_nNextBankerIdx = 0;
	}
	else {
...
void ARQMJRoom::onPlayerMo(uint8_t nIdx) {
	IMJRoom::onPlayerMo(nIdx);
	auto pPlayer = (ARQMJPlayer*)getPlayerByIdx(nIdx);
	pPlayer->clearGangFlag();
	pPlayer->signFlag(IMJPlayer::eMJActFlag::eMJActFlag_NeedClearCanCyclone);
}

void ARQMJRoom::onPlayerChu(uint8_t nIdx, uint8_t nCard) {
	auto pPlayer = (ARQMJPlayer*)getPlayerByIdx(nIdx);
...
void ARQMJRoom::onPlayerEat(uint8_t nIdx, uint8_t nCard, uint8_t nWithA, uint8_t nWithB, uint8_t nInvokeIdx) {
...
void ARQMJRoom::onPlayerPeng(uint8_t nIdx, uint8_t nCard, uint8_t nInvokeIdx) {
...
void ARQMJRoom::onPlayerMingGang(uint8_t nIdx, uint8_t nCard, uint8_t nInvokeIdx) {
	onPrePlayerGang();
...
void ARQMJRoom::onPlayerAnGang(uint8_t nIdx, uint8_t nCard) {
	onPrePlayerGang();
...
void ARQMJRoom::onPlayerCyclone(uint8_t nIdx, uint8_t nCard) {
	...
	if (((ARQMJPlayerCard*)pPlayer->getPlayerCard())->onCyclone(nCard, nGangGetCard) == false)
	{
...
void ARQMJRoom::onPlayerBuGang(uint8_t nIdx, uint8_t nCard) {
	auto pPlayer = (ARQMJPlayer*)getPlayerByIdx(nIdx);
	if (pPlayer) {
		stSettle st;
...
void ARQMJRoom::onPlayerHu(std::vector<uint8_t>& vHuIdx, uint8_t nCard, uint8_t nInvokeIdx) {
	if (vHuIdx.empty())
	{
	...
	if (isZiMo) {
		...
		auto pZiMoPlayer = (ARQMJPlayer*)getPlayerByIdx(nInvokeIdx);
		...
		uint16_t nFanCnt = 0;
		auto pZiMoPlayerCard = (ARQMJPlayerCard*)pZiMoPlayer->getPlayerCard();
		...
		for (auto& pLosePlayer : m_vPlayers)
		{
			if (pLosePlayer) {
				...
				if (m_cFanxingChecker.checkFanxing(eFanxing_MengQing, (ARQMJPlayer*)pLosePlayer, nInvokeIdx, this)) {
					nTLoseCoin *= 2;
				}
		...
		auto pLosePlayer = (ARQMJPlayer*)getPlayerByIdx(nInvokeIdx);
		if (!pLosePlayer)
		{
		...
		{
			auto pHuPlayer = (ARQMJPlayer*)getPlayerByIdx(nHuIdx);
			...
			auto pHuPlayerCard = (ARQMJPlayerCard*)pHuPlayer->getPlayerCard();
			...
			for (auto& ref : m_vPlayers) {
				if (ref && ref != pHuPlayer) {
					...
					if (m_cFanxingChecker.checkFanxing(eFanxing_MengQing, (ARQMJPlayer*)ref, nInvokeIdx, this)) {
						nPCoin *= 2;
					}
	...
	/*for (auto huIdx : vHuIdx) {
		auto pHuPlayer = (ARQMJPlayer*)getPlayerByIdx(huIdx);
		if (pHuPlayer == nullptr)
		{
			LOGFMTE("room id = %u hu player idx = %u , is nullptr", getRoomID(), huIdx);
			continue;
		}
		auto pHuCard = (ARQMJPlayerCard*)pHuPlayer->getPlayerCard();
...
void ARQMJRoom::onWaitPlayerAct(uint8_t nIdx, bool& isCanPass) {
	auto pPlayer = (IMJPlayer*)getPlayerByIdx(nIdx);
	if (!pPlayer) {
	...
	auto pMJCard = (ARQMJPlayerCard*)pPlayer->getPlayerCard();
...
bool ARQMJRoom::onWaitPlayerActAfterCP(uint8_t nIdx) {
	auto pPlayer = (IMJPlayer*)getPlayerByIdx(nIdx);
	if (!pPlayer) {
		LOGFMTE("player idx = %u is null can not tell it wait act", nIdx);
		return false;
...
void ARQMJRoom::onAskForPengOrHuThisCard(uint8_t nInvokeIdx, uint8_t nCard, std::vector<uint16_t>& vOutWaitHuIdx, std::vector<uint16_t>& vOutWaitPengGangIdx, bool& isNeedWaitEat) {
	Json::Value jsFrameArg;
...
bool ARQMJRoom::isAnyPlayerRobotGang(uint8_t nInvokeIdx, uint8_t nCard) {
	for (auto& ref : m_vPlayers)
	{
		if (ref == nullptr || nInvokeIdx == ref->getIdx())
		{
...
void ARQMJRoom::onAskForRobotGang(uint8_t nInvokeIdx, uint8_t nCard, std::vector<uint8_t>& vOutCandinates) {
...
uint8_t ARQMJRoom::getNextActPlayerIdx(uint8_t nCurActIdx) {
	return (nCurActIdx + 1) % getSeatCnt();
}

//uint8_t ARQMJRoom::getFanLimit() {
//	auto pARQMJOpts = std::dynamic_pointer_cast<ARQMJOpts>(getDelegate()->getOpts());
//	return pARQMJOpts->getFanLimit();
//}
//
//bool ARQMJRoom::isDPOnePay() {
//	auto pARQMJOpts = std::dynamic_pointer_cast<ARQMJOpts>(getDelegate()->getOpts());
//	return pARQMJOpts->isDianPaoOnePay();
//}
//
//uint32_t ARQMJRoom::getGuang() {
//	auto pARQMJOpts = std::dynamic_pointer_cast<ARQMJOpts>(getDelegate()->getOpts());
//	return pARQMJOpts->getGuang();
//}

void ARQMJRoom::addSettle(stSettle& tSettle) {
	...
	{
		auto pPlayer = (ARQMJPlayer*)getPlayerByIdx(refl.first);
		if (pPlayer) {
...
void ARQMJRoom::settleInfoToJson(Json::Value& jsRealTime) {
...
void ARQMJRoom::sortFanxing2FanCnt(std::vector<eFanxingType>& vType, uint16_t& nFanCnt) {
	for (auto& ref : vType) {
		switch (ref)
		{
...
void ARQMJRoom::addGain(uint8_t nIdx, stSettleGain stGain) {
	if (nIdx < m_vGainChip.size()) {
		m_vGainChip[nIdx].push_back(stGain);
	}
}

void ARQMJRoom::clearGain() {
	for (auto& ref : m_vGainChip) {
		ref.clear();
	}
}

void ARQMJRoom::backGain(uint8_t nIdx) {
	if (nIdx < m_vGainChip.size()) {
		...
		auto pLosePlayer = (ARQMJPlayer*)getPlayerByIdx(nIdx);
		...
		for (auto& ref : vGain) {
			...
			auto pTargetPlayer = (ARQMJPlayer*)getPlayerByIdx(ref.nTargetIdx);
...
bool ARQMJRoom::isPlayerRootDirectGang(uint8_t nInvokerIdx, uint8_t nCard) {
...
void ARQMJRoom::onAskForRobotDirectGang(uint8_t nInvokeIdx, uint8_t nActIdx, uint8_t nCard, std::vector<uint8_t>& vOutCandinates) {
	// send decalre gang msg ;
	Json::Value msg;
	msg["idx"] = nInvokeIdx;
	...
	// inform target player do this things 
	auto pPlayer = (ARQMJPlayer*)getPlayerByIdx(nActIdx);
...
bool ARQMJRoom::onPlayerEnter(stEnterRoomData* pEnterRoomPlayer)
{
...
void ARQMJRoom::doRandomChangeSeat() {
...
bool ARQMJRoom::doChangeSeat(uint16_t nIdx, uint16_t nWithIdx) {
...
uint8_t ARQMJRoom::checkPlayerCanSitDown(stEnterRoomData* pEnterRoomPlayer)
{
