...
#include "ARQMJFanxingGangHouPao.h"
#include "FanxingMenQing.h"
#include "ARQMJFanxingHaiDiLaoYue.h"
#include "ARQMJFanxingJiaHu.h"
#include "ARQMJFanxingBianHu.h"
class ARQMJFanxingChecker
...
public:
	void init() {
		...
		addFanxing(new ARQMJFanxingJiaHu());
		addFanxing(new ARQMJFanxingBianHu());
		addFanxing(new FanxingQiangGang());
		addFanxing(new ARQMJFanxingGangHouPao());
		addFanxing(new ARQMJFanxingHaiDiLaoYue());
	...
	void checkFanxing(std::vector<eFanxingType>& vFanxing, IMJPlayer* pPlayer, uint8_t nInvokerIdx, IMJRoom* pmjRoom)override {
		bool isDDH = false;
		bool isBian = false;
		auto pCard = (ARQMJPlayerCard*)pPlayer->getPlayerCard();
