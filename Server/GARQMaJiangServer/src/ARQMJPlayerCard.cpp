...
bool ARQMJPlayerCard::canEatCard(uint8_t nCard, uint8_t nWithA, uint8_t nWithB) {
	VEC_CARD vCards;
	getHoldCard(vCards);
	if (vCards.size() < 5) {
		return false;
...
bool ARQMJPlayerCard::canPengWithCard(uint8_t nCard) {
	VEC_CARD vHoldCards;
...
bool ARQMJPlayerCard::canMingGangWithCard(uint8_t nCard) {
	VEC_CARD vHoldCards;
	getHoldCard(vHoldCards);
	if (vHoldCards.size() < 5) {
		VEC_CARD vTemp;
...
bool ARQMJPlayerCard::canAnGangWithCard(uint8_t nCard) {
	VEC_CARD vHoldCards;
	getHoldCard(vHoldCards);
...
void ARQMJPlayerCard::onVisitPlayerCardInfo(Json::Value& js, bool isSelf) {
	MJPlayerCard::onVisitPlayerCardInfo(js, isSelf);
}

bool ARQMJPlayerCard::getHoldCardThatCanAnGang(VEC_CARD& vGangCards) {
	VEC_CARD vHoldCards;
	getHoldCard(vHoldCards);
	if (vHoldCards.size() < 6) {
		VEC_CARD vTemp;
...
bool ARQMJPlayerCard::getHoldCardThatCanBuGang(VEC_CARD& vGangCards) {
...
bool ARQMJPlayerCard::canCycloneWithCard(uint8_t nCard) {
	VEC_CARD vHoldCards;
	getHoldCard(vHoldCards);
	if (vHoldCards.size() < 6) {
		VEC_CARD vTemp;
...
bool ARQMJPlayerCard::getHoldCardThatCanCyclone(VEC_CARD& vGangCards) {
	VEC_CARD vHoldCards;
	getHoldCard(vHoldCards);
	if (vHoldCards.size() < 6) {
		VEC_CARD vTemp;
...
bool ARQMJPlayerCard::onCyclone(uint8_t nCard, uint8_t nGangGetCard) {
...
bool ARQMJPlayerCard::getCycloneCard(VEC_CARD& vGangCard)
{
	for (auto ref : m_vMingCardInfo)
	{
...
bool ARQMJPlayerCard::isHoldCardCanHu() {
	return isHoldCardCanHu(m_nJIang);
}

bool ARQMJPlayerCard::isHoldCardCanHu(uint8_t& nJiang) {
...
bool ARQMJPlayerCard::isTingPai() {
	return MJPlayerCard::isTingPai();
}

//bool ARQMJPlayerCard::isHoldCardCanHu(VEC_CARD vTemp) {
...
//}
//
//bool ARQMJPlayerCard::isHoldCardCanHu(VEC_CARD vTemp[eCT_Max]) {
//	VEC_CARD vAllCards[eCT_Max];
//	for (uint8_t i = eCT_None; i < eCT_Max; i++) {
...
bool ARQMJPlayerCard::isHaveCards(VEC_CARD vCards) {
	VEC_CARD vTemp;
	getHoldCard(vTemp);
	for (auto ref : vCards) {
...
bool ARQMJPlayerCard::eraseVector(uint8_t p, VEC_CARD& typeVec)
{
...
bool ARQMJPlayerCard::checkKezi() {
	VEC_CARD vTemp;
...
bool ARQMJPlayerCard::check3Men() {
	VEC_CARD vTemp;
	getEatedCard(vTemp);
	getPengedCard(vTemp);
...
bool ARQMJPlayerCard::isDanDiao() {
	if (getHoldCardCnt() < 3) {
		return true;
	}
	/*if (canHuOnlyOneCard() == false) {
...
bool ARQMJPlayerCard::canHuOnlyOneCard() {
...
bool ARQMJPlayerCard::isHuOnly19() {
	auto nCard = getHuCard();
	if (isHaveCard(nCard) == false) {
		return false;
	}
...
uint8_t ARQMJPlayerCard::getHoldCardCnt() {
	VEC_CARD vHoldCard;
