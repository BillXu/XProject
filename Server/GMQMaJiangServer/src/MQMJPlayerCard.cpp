...
bool MQMJPlayerCard::canMingGangWithCard(uint8_t nCard) {
	VEC_CARD vHoldCards;
	getHoldCard(vHoldCards);
	if (vHoldCards.size() < 5) {
		VEC_CARD vTemp;
...
bool MQMJPlayerCard::canAnGangWithCard(uint8_t nCard) {
	VEC_CARD vHoldCards;
	getHoldCard(vHoldCards);
...
void MQMJPlayerCard::onVisitPlayerCardInfo(Json::Value& js, bool isSelf) {
	MJPlayerCard::onVisitPlayerCardInfo(js, isSelf);
}

bool MQMJPlayerCard::getHoldCardThatCanAnGang(VEC_CARD& vGangCards) {
	VEC_CARD vHoldCards;
	getHoldCard(vHoldCards);
	if (vHoldCards.size() < 6) {
		VEC_CARD vTemp;
...
bool MQMJPlayerCard::getHoldCardThatCanBuGang(VEC_CARD& vGangCards) {
...
bool MQMJPlayerCard::canCycloneWithCard(uint8_t nCard) {
	VEC_CARD vHoldCards;
	getHoldCard(vHoldCards);
	if (vHoldCards.size() < 6) {
		VEC_CARD vTemp;
...
bool MQMJPlayerCard::onCyclone(uint8_t nCard, uint8_t nGangGetCard) {
...
bool MQMJPlayerCard::getCycloneCard(VEC_CARD& vGangCard)
{
	for (auto ref : m_vMingCardInfo)
	{
...
bool MQMJPlayerCard::isHoldCardCanHu() {
	return isHoldCardCanHu(m_nJIang);
}

bool MQMJPlayerCard::isHoldCardCanHu(uint8_t& nJiang) {
...
bool MQMJPlayerCard::eraseVector(uint8_t p, VEC_CARD& typeVec)
{
...
bool MQMJPlayerCard::canHuOnlyOneCard() {
...
bool MQMJPlayerCard::isHuOnly19() {
	auto nCard = getHuCard();
	if (isHaveCard(nCard) == false) {
		return false;
	}
...
uint8_t MQMJPlayerCard::getHoldCardCnt() {
	VEC_CARD vHoldCard;
