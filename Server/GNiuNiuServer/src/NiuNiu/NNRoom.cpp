...
void NNRoom::visitPlayerInfo( IGamePlayer* pPlayer, Json::Value& jsPlayerInfo,uint32_t nVisitorSessionID)
{
	...
	if ( 1 || nVisitorSessionID == pPlayer->getSessionID())
	{
		...
		if ( eRoomState_DoBet == getCurState()->getStateID() && eDecideBank_LookCardThenRobot == m_eDecideBankerType && ((NNPlayer*)pPlayer)->isRobotBanker() == false )
		{
...
uint8_t NNRoom::onPlayerDoBet( uint16_t nIdx, uint8_t nBetTimes )
{
	...
	if (nBetTimes > (getMiniBetTimes() * 2) && (p->getIdx() == m_nLastBankerIdx || p->getLastOffset() <= 0 || p->isLastTuiZhu() || (m_eDecideBankerType == eDecideBank_LookCardThenRobot && p->isRobotBanker() == false )))
	{
		return 5;
	}
