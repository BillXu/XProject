...
protected:
	void caculateResult()
	{
		...
		{
			auto p = (GDPlayer*)pRoom->getPlayerByIdx(nIdx);
			if (p == nullptr)
			{
		...
		auto pBanker = (GDPlayer*)pRoom->getPlayerByIdx(pRoom->getBankerIdx());
		if ( nullptr == pBanker)
		{
		...
		for (uint8_t nIdx = 0; nIdx < pRoom->getSeatCnt(); ++nIdx)
		{
			auto pPlayer = (GDPlayer*)pRoom->getPlayerByIdx(nIdx);
			if (pPlayer == pBanker)
			{
				continue;
			}
		...
		// add frame 
		pRoom->addReplayFrame(GD_Frame_GameEnd, jsMsg);

		pRoom->sendRoomMsg(jsMsg,MSG_GD_ROOM_RESULT);
	}
};