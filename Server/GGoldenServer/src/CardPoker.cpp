...
unsigned char CCard::GetCardCompositeNum() const
{
...
// Poker
void CGoldenPoker::init( Json::Value& jsOpts )
{
	uint8_t nCardCnt = 52;
	if (jsOpts["joker"].isNull() == false && jsOpts["joker"].asUInt() == 1)
	{
		nCardCnt = 54;
		LOGFMTD("create bai bian card ");
	}

	uint8_t nParse = jsOpts["parseUntil"].asUInt();

	for ( int i = 1 ; i <= nCardCnt; ++i )
	{
		if (nParse) {
			auto cCard = CCard(i);
			if (cCard.GetCardFaceNum() < nParse) {
				continue;
			}
		}
		addCardToPoker(i) ;
