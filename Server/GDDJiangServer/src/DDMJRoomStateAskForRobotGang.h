...
#include "DDMJRoom.h"
class DDMJRoomStateAskForRobotGang
...
public:
	void enterState(GameRoom* pmjRoom, Json::Value& jsTranData)override
	{
		...
		if (m_nActType == eMJAct_MingGang_Pre) {
			pRoom->onAskForRobotDirectGang(m_nInvokeIdx, m_nActIdx, m_nCard, m_vWaitIdx);
		}
		else {
	...
	void update(float fDeta)override
	{
		if (getWaitTime() > 15.0f) {
			for (auto& ref : m_vWaitIdx) {
				auto pPlayer = (DDMJPlayer*)getRoom()->getPlayerByIdx(ref);
	...
	bool onMsg(Json::Value& prealMsg, uint16_t nMsgType, eMsgPort eSenderPort, uint32_t nSessionID)override
	{
		...
		auto pPlayer = (DDMJPlayer*)getRoom()->getPlayerBySessionID(nSessionID);
		uint8_t nRet = 0;
		do
		{
			if (pPlayer == nullptr)
