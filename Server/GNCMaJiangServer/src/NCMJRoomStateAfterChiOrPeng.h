...
class NCMJRoomStateAfterChiOrPeng
...
public:
	...
	void enterState(GameRoom* pmjRoom, Json::Value& jsTranData)override
	{
		...
		if (jsTranData["idx"].isNull() == false && jsTranData["idx"].isUInt())
		{
			...
			if (((NCMJRoom*)getRoom())->onWaitPlayerActAfterCP(m_nIdx)) {
				//TODO...
			}
			else {
				Json::Value jsValue;
	...
	void update(float fDeta)override
	{
		if (getWaitTime() > 15.0f) {
			auto pPlayer = (NCMJPlayer*)getRoom()->getPlayerByIdx(m_nIdx);
	...
	bool onMsg(Json::Value& prealMsg, uint16_t nMsgType, eMsgPort eSenderPort, uint32_t nSessionID)override
	{
		...
		auto pPlayer = (NCMJPlayer*)getRoom()->getPlayerBySessionID(nSessionID);
		uint8_t nRet = 0;
		do
		{
			if (m_bPass) {
			...
			auto pMJCard = (NCMJPlayerCard*)pPlayer->getPlayerCard();
			switch (actType)
			{
			case eMJAct_AnGang:
			{
